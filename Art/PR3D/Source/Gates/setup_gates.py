...
UNITY_GATE_DIR = REPO_ROOT / "Assets/_Projects/Art/PR3D/Gates"
EVIDENCE_DIR = UNITY_GATE_DIR / "Evidence"
...
PREVIEW = EVIDENCE_DIR / "PR3D_005_GateVariants.png"
MANIFEST = UNITY_GATE_DIR / "PR3D_Gates_manifest.json"
...
def build_shared_mesh(
    ...
    shared["source_bounds_m"] = "1.104 x 0.316 x 0.790"
...
def write_manifest(shared: bpy.types.Object) -> None:
    ...
    payload = {
        "task": "PR3D-005",
        "contract": {
            "usage": "visual-only child; do not replace gameplay root or anchors",
            "source_root": "PR3D_Gate_Root",
            "source_pivot_m": [0.0, 0.0, 0.0],
            "source_entry_local_m": [0.0, 0.25, 0.0],
            "source_exit_local_m": [0.0, -0.25, 0.0],
            "source_bounds_xyz_m": [1.104, 0.316, 0.790],
            "unity_forward": "+Z after the FBX root axis correction",
            "unity_asset_root": "PR3D_Gate_Shared",
            "unity_import_global_scale": 0.9057971,
            "unity_import_root_euler_deg": [270.0, 0.0, 0.0],
            "unity_import_entry_local_m": [0.0, 0.226, 0.0],
            "unity_import_exit_local_m": [0.0, -0.226, 0.0],
            "unity_import_bounds_xyz_m": [1.000, 0.716, 0.286],
            "production_line_root": {
                "local_position_m": [0.0, 0.0, 0.0],
                "local_rotation_deg": [0.0, 0.0, 0.0],
                "local_scale": [1.0, 1.0, 1.0],
            },
            "observed_production_place_offsets_m": [0.520, 0.187, -0.143],
        },
        "shared_mesh": {
            "name": shared.data.name,
            "vertices": len(shared.data.vertices),
            "polygons": len(shared.data.polygons),
            "uv_layers": [layer.name for layer in shared.data.uv_layers],
        },
        "variants": list(COLORS),
        "materials": [
            f"Materials/PR3D_MAT_GateVariant_{name}.mat" for name in COLORS
        ],
        "exports": {
            "fbx": {
                "path": "Models/PR3D_Gate_Shared.fbx",
                "sha256": sha256(UNITY_FBX),
            },
            "glb_source_export": {
                "path": "Art/PR3D/Exports/Gates/PR3D_Gate_Shared.glb",
                "sha256": sha256(EXPORT_GLB),
            },
        },
        "evidence": "Evidence/PR3D_005_GateVariants.png",
        "archive_reuse": {
            "source_commit": "f303b8c",
            "scope": "Gate family files only",
            "revalidated": True,
        },
    }
...
def main() -> None:
    for directory in (SOURCE_DIR, EXPORT_DIR, UNITY_MODEL_DIR, EVIDENCE_DIR):
